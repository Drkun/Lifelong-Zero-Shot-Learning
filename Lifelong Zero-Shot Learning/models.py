...
class encoder_att(nn.Module):
    def __init__(self, input_dim, output_dim, device):
        ...
        modules = []
        modules.append(nn.Linear(in_features=input_dim, out_features=1450))
        modules.append(nn.ReLU())
        self.feature_encoder = nn.Sequential(*modules)
        self._mu = nn.Linear(in_features=1450, out_features=output_dim)
        self._logvar = nn.Linear(in_features=1450, out_features=output_dim)
        self.to(device)
    def forward(self, x):
...
class encoder_template(nn.Module):
    def __init__(self,input_dim,latent_size,hidden_size_rule,device):
        ...
        for i in range(len(self.layer_sizes)-2):
           # print('encoder dim:',self.layer_sizes[i+1])
